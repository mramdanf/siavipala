...
<body>
  <div id="top-section">
    <h2>LAPORAN KEGIATAN HARIAN PATROLI TERPADU & POSKO PENCEGAHAN KARHUTLA PROVINSI <?=strtoupper($provinsi['nama'])?></h2>
    <h3>Tanggal: <?=$tanggal?></h3>
  </div>

  <?php foreach($resume_per_daops_perday as $resume_per_daops): ?>
  
  <h3>Daops <?=ucwords($resume_per_daops['daops']['nama'])?></h3>
  <hr style="color: grey;">

  <div id="pelaksana">
    <h3>1. Pelaksana</h3>
    <table>
      <?php foreach($resume_per_daops['kategoriAnggota'] as $ka): ?>
      <tr>
        <td><?=$ka['nama']?></td>
        <td>:</td>
  ...
  <div id="posko">
    ...
    <table class="table-padding table-bordered">
      ...
      <tbody>
        <?php foreach($resume_per_daops['kegiatanPatroli'] as $itemKegiatanPatroli): ?>
  ...
  <div id="kegiatan-patroli">
    ...
    <table class="table-padding table-bordered">
      ...
      <tbody>
        <?php $noKegiatanPatroli = 0; foreach($resume_per_daops['kegiatanPatroli'] as $itemKegiatanPatroli): ?>
          <?php foreach($itemKegiatanPatroli['patroli_darat'] as $key => $pd): ?>
          <tr>
            <!-- No -->
  ...
  <div id="pemadaman">
    <h3>5. Pemadaman</h3>
    <?php $noPemadaman = 0; foreach($resume_per_daops['kegiatanPatroli'] as $itemKegiatanPatroli): ?>
